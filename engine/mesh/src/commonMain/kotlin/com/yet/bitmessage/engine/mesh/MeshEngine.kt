...
class MeshEngine(
    ...
    override fun reduce(
        ...
        when (event) {
            ...
            is MeshEvent.PacketDigestComputed -> ignoredResult(state, event)
            is MeshEvent.SignatureVerified -> ignoredResult(state, event)
            is MeshEvent.FragmentPayloadDecoded -> ignoredResult(state, event)
            is MeshEvent.RelayEncoded -> ignoredResult(state, event)
            is MeshEvent.EntropyProvided -> ignoredResult(state, event)
            is MeshEvent.TimerElapsed -> ignoredResult(state, event)
            is MeshEvent.LinkCompleted -> ignoredResult(state, event)
