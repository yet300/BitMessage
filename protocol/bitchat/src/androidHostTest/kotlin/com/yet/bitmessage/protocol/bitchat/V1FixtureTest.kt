...
class V1FixtureTest {
    @Test
    fun everyResolvedV1OuterFixtureDecodesAndReencodesExactly() {
        val fixtures = FixtureManifestParser.parse(resource("BitchatBaseline2026_08/fixtures.json")).fixtures
            .filter { it.id in V1_FIXTURE_IDS }

        assertEquals(V1_FIXTURE_IDS, fixtures.map { it.id }.toSet())
    ...
    private companion object {
        val V1_FIXTURE_IDS = setOf(
            "apple-v1-broadcast",
            "apple-v1-recipient",
            "android-v1-broadcast",
            "android-v1-recipient",
        )
    }
}
