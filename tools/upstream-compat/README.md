...
This harness recreates the Phase 1 `BitchatBaseline2026_08` producer and reciprocal-acceptance evidence. It is deliberately excluded from ordinary PR CI: the checked-in neutral fixtures under `compatibility/` are the offline CI input.
...
`prepare-upstreams.sh` refuses a checkout whose `HEAD` is not the pinned SHA. `install-harness.sh` copies only test sources into the disposable clones. `verify-upstreams.sh` executes four distinct evidence paths:
