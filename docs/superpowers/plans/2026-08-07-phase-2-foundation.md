...
Status: COMPLETE (2026-08-07). Phase 2 finished without starting Phase 3.
...
- [x] Add `:core:foundation` and `:core:model` beside the existing `:core:common` and `:core:testing` projects.
- [x] Apply only `libs.plugins.local.kotlin.multiplatform` in each new module.
- [x] Give `:core:model` a `commonMain` dependency on `projects.core.foundation`, and give `:core:testing` a `commonMain` dependency on `projects.core.foundation`.
...
```
- [x] Run `rtk ./gradlew projects` and verify the three core modules appear without a circular dependency.
- [x] Update the repository map and dependency graph in `AGENTS.md`.
...
- [x] Write a failing `BytesTest` proving copied input/output ownership, unsigned access, content equality/hash behavior, and explicit exact-size failure for short and long input.
...
```
- [x] Run `rtk ./gradlew :core:foundation:allTests`; confirm compilation fails because `Bytes` is absent.
- [x] Implement `Bytes` with private `ByteArray` storage, `copyOf`, `requireExactSize`, `copyToByteArray`, `get(index): UByte`, content equality/hashCode, and size-only `toString`.
...
```
- [x] Run `rtk ./gradlew :core:foundation:allTests`; confirm the byte tests pass.
- [x] Write failing model tests proving `LinkId` and `PeerId` have separate factory APIs, reject blank/empty input, preserve bytes without normalization, and cannot be exchanged at Kotlin compile time.
- [x] Implement `@JvmInline` `LinkId` and `PeerId`; keep `PeerId` opaque and unconstrained by any BitChat byte length.
- [x] Add `TimerId`, `CorrelationId`, and `Generation` to foundation; reject blank tokens and negative generation values. Do not create `IdentityId`, `SessionGeneration`, message, attempt, or operation IDs.
...
```
- [x] Run `rtk ./gradlew :core:model:allTests :core:foundation:allTests`.
...
- [x] Write failing virtual-runtime tests for explicit virtual-time advancement, equal-deadline sequence order, cancellation, replacement, stale generation distinction, and same-seed entropy replay.
...
```
- [x] Run `rtk ./gradlew :core:testing:allTests`; confirm the test compilation fails before virtual runtime types exist.
- [x] Implement `WallClock`, `MonotonicClock`, `MonotonicTime`, `ScheduleTimer`, `CancelTimer`, `TimerFired`, `EntropyRequest`, `EntropyGenerated`, and `EntropySource` in foundation without a production scheduler or entropy source.
...
```
- [x] Implement virtual clocks, a deadline/sequence scheduler, and a test-only seeded entropy source in `:core:testing`. The scheduler must not call sleep or a wall clock.
- [x] Run `rtk ./gradlew :core:testing:allTests` and confirm all virtual-runtime tests pass on Android and iOS.
...
- [x] Write a failing reducer test with an immutable counter state and event that asserts repeated `reduce` calls return identical state, effect ordering, and trace records.
- [x] Write a failing trace test that verifies a trace contains its transition/correlation/size/decision facts but never exposes the bytes supplied to an entropy result.
- [x] Run `rtk ./gradlew :core:foundation:allTests`; confirm the tests fail because `Engine`, `Transition`, and trace types do not exist.
- [x] Implement only `Engine`, `Transition`, `TraceRecord`, `TransitionName`, `TraceDecision`, and nonnegative `TraceSize`; do not add composition or execution machinery.
...
```
- [x] Run `rtk ./gradlew :core:foundation:allTests` and confirm deterministic reducer and redaction tests pass.
...
- Modify: this checklist/status; no additional `AGENTS.md` change is required after Task 1.

- [x] Mark Tasks 2.1–2.4 complete only after implementation and tests establish the stated boundaries and laws.
- [x] State that `IdentityId` and `SessionGeneration` are deferred to Phase 7, while `PeerId` remains transport-visible rather than durable identity.
- [x] Run `rtk ./gradlew :core:testing:compatibilityCheck :core:foundation:allTests :core:model:allTests :core:testing:allTests :androidApp:assembleDebug :sharedLogic:linkDebugFrameworkIosSimulatorArm64`.
- [x] Inspect Android-host and iOS test XML: foundation executed 14/14, model 3/3, and testing 32/26 tests (Android-host/iOS Simulator); the compatibility gate's 32 Android-host tests include 6 corpus tests. No selected suite is `NO-SOURCE`; `rtk git diff --check` passes. Code-graph re-index was attempted three times, but the MCP transport closed before dispatch; this is recorded as an external tooling issue.
- [x] Commit the verified Phase 2 change on `phase-1-compatibility-baseline` without merging or starting Phase 3.
