...
Status: architecture implemented through the Phase 5 deterministic multi-node mesh simulator
...
| `:transport:simulation` | Implemented bounded multi-node virtual network around one real `MeshEngine`/`MeshRuntime` per node. Allowed production dependencies: `:core:foundation`, `:core:model`, `:protocol:bitchat`, `:transport:api`, `:engine:mesh`; `:core:testing` is test-only. Forbidden: application/sharedLogic reverse dependency, platform BLE, Noise, persistence, Delivery/Sync, and UI. API: `SimulatedNetwork`, directed links, explicit `FaultPlan`, scheduled `ScenarioAction`. Tests: Android-host/iOS Simulator scenarios, 64 generated fixed seeds, replay equality, and non-zero `simulationCheck`. Source sets: commonMain/commonTest/androidHostTest. |
...
Phase 2 introduced the generic reducer kernel below. Phase 4 implements its first production use, `MeshEngine`, plus the separate `MeshRuntime` imperative shell. Phase 5 adds only test/simulation infrastructure around those real components. There is still no production entropy provider, physical transport, cryptographic implementation, persistence, or application composition for the mesh path. Test-only `VirtualScheduler` and non-cryptographic `SeededEntropy` remain in `:core:testing`:
...
`MeshEngine.reduce` commits immutable `MeshState` and emits ordered `MeshEffect` and redacted trace records. The runtime protocol adapter structurally decodes link bytes before the reducer. SHA-256, signature verification, entropy, timers, relay encoding, link writes, and public-payload publication are effect boundaries; their correlated `MeshEvent` results carry `Generation` and explicit `MonotonicTime`. `MeshRuntime` creates no jobs or channels in its constructor, then on `start` creates one reducer actor, one ordered effect worker, bounded event/effect/trace channels, and correlated timer jobs. `trySubmit` reports `Accepted`, `Backpressured`, or `Closed`; stop retains only unexpired admitted packet IDs, later start increments generation, and close is permanent. Detailed contracts, bounds, TTL, relay, fragment, and lifecycle behavior are in `STATE_MACHINE_DESIGN.md`.

The Phase 5 simulator owns one authoritative scheduled virtual-event queue ordered by `(deadline, insertionSequence)`; it does not replace each runtime's bounded actor/effect queues. A runtime acknowledgement means an input transition committed and its resulting effects were registered/submitted, **not** that asynchronous effects completed. A causal fence and `runCurrentUntilQuiescent` drain current-instant mailbox work, effects/results, and same-time events without advancing into future maintenance timers. `advanceTo`/`advanceBy` move virtual time explicitly; predicate-bounded `runUntil` advances only until its condition is true and drains that instant. Passive future expiry timers do not block protocol convergence. Scenario compilation uses a separate seeded RNG from each node's protocol entropy, emits an explicit `FaultPlan` and scheduled actions before execution, and supports exact replay without a shrinker. The default simulation digest is real SHA-256 of the Phase 4 canonical packet-identity input; `DigestPlan` overrides only named exceptional cases. The canonical `compatibility/` corpus remains the sole source of compatibility literals. The general TTL cap of 7 and hostile `255 -> 6` mutation remain local policy; only concrete `7 -> 6` is dual-upstream evidence.
